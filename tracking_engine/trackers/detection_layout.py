...
class DetectionLayout:
    """Định nghĩa index và shape chuẩn cho tensor detection của tracker."""
    ...
    def boxes(self, dets: np.ndarray) -> np.ndarray:
        if dets.size == 0:
            ...
            return np.empty((0, self.box_cols), dtype=dtype)
    ...
    def with_detection_indices(self, dets: np.ndarray) -> np.ndarray:
        if dets.size == 0:
            dtype = dets.dtype if hasattr(dets, "dtype") else np.float32
            return np.empty((0, self.det_cols + 1), dtype=dtype)
...
class AxisAlignedDetections(DetectionLayout):
    """Layout cho detection dạng AABB: (x1, y1, x2, y2, conf, cls)."""
...
class OrientedDetections(DetectionLayout):
    """Layout cho detection dạng OBB: (cx, cy, w, h, angle, conf, cls)."""
