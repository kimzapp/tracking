...
struct TrackerRuntimeConfig {
  ...
  float new_track_thresh = 0.75F;
  int track_buffer = 450;
  float match_thresh = 0.72F;
  float proximity_thresh = 0.55F;
  float appearance_thresh = 0.32F;
  bool reid_recovery_enabled = true;
  float reid_recovery_proximity_thresh = 0.88F;
  float reid_recovery_appearance_thresh = 0.35F;
  float reid_recovery_thresh = 0.40F;
  float second_association_thresh = 0.35F;
